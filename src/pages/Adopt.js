...
class Adopt extends React.Component {
	...
	async componentDidMount() {
		...
		try {
			...
			const pets = res.map((item) => ({
				...
				rfid: item.rfidTag,
				petId: item.id,
				type: item.personal.type,
				age: {
					year: item.personal.age.year,
					month: item.personal.age.month
				},
				size: item.background.size,
			}));
			...
			searchInput.addEventListener('input', () => {
				...
				this.setState({ displayedPets: filteredPets });
			});
...

			{
				...
				cat.addEventListener('change', () => {
					if (cat.checked) {
						...
						this.setState({ displayedPets: pets });
					};
				});
				...
				// Size
				// If small, medium, or large is checked, filter pets by size
				...
				small.addEventListener('change', () => {
					if (small.checked) {
						if (medium.checked) {
							const filteredPets = pets.filter((pet) => pet.size === 'Small' || pet.size === 'Medium');
							this.setState({ displayedPets: filteredPets });
						} else if (large.checked) {
							const filteredPets = pets.filter((pet) => pet.size === 'Small' || pet.size === 'Large');
							this.setState({ displayedPets: filteredPets });
						} else {
							const filteredPets = pets.filter((pet) => pet.size === 'Small');
							this.setState({ displayedPets: filteredPets });
						};
					} else {
						if (medium.checked) {
							const filteredPets = pets.filter((pet) => pet.size === 'Medium');
							this.setState({ displayedPets: filteredPets });
						} else if (large.checked) {
							const filteredPets = pets.filter((pet) => pet.size === 'Large');
							this.setState({ displayedPets: filteredPets });
						} else {
							this.setState({ displayedPets: pets });
						};
				...
				medium.addEventListener('change', () => {
					if (medium.checked) {
						if (small.checked) {
							...
							const filteredPets = pets.filter((pet) => pet.size === 'Medium' || pet.size === 'Large');
							this.setState({ displayedPets: filteredPets });
						} else {
							const filteredPets = pets.filter((pet) => pet.size === 'Medium');
							this.setState({ displayedPets: filteredPets });
				...
				const oneToFive = document.getElementById('1-5years');
				const fivePlus = document.getElementById('5+years');

				months.addEventListener('change', () => {
					if (months.checked) {
						if (oneToFive.checked) {
							const filteredPets = pets.filter((pet) => pet.age.month || pet.age.year <= 5);
							this.setState({ displayedPets: filteredPets });
						} else if (fivePlus.checked) {
							const filteredPets = pets.filter((pet) => pet.age.month);
							this.setState({ displayedPets: filteredPets });
						} else {
							const filteredPets = pets.filter((pet) => pet.age.month);
							this.setState({ displayedPets: filteredPets });
						...
						if (oneToFive.checked) {
							const filteredPets = pets.filter((pet) => pet.age.year <= 5);
							this.setState({ displayedPets: filteredPets });
						} else if (fivePlus.checked) {
							const filteredPets = pets.filter((pet) => pet.age.year > 5);
							this.setState({ displayedPets: filteredPets });
				...
				oneToFive.addEventListener('change', () => {
					if (oneToFive.checked) {
						if (months.checked) {
							const filteredPets = pets.filter((pet) => pet.age.month || pet.age.year <= 5);
							this.setState({ displayedPets: filteredPets });
				...
				fivePlus.addEventListener('change', () => {
					if (fivePlus.checked) {
						if (months.checked) {
							const filteredPets = pets.filter((pet) => pet.age.month);
							this.setState({ displayedPets: filteredPets });
						} else if (oneToFive.checked) {
							const filteredPets = pets.filter((pet) => pet.age.year <= 5);
							this.setState({ displayedPets: filteredPets });
						} else {
							const filteredPets = pets.filter((pet) => pet.age.year > 5);
	...
	render() {
		const { displayedPets, loaded } = this.state;
		...
		return (
			<>
					...
					<div>
						<div id="searchBar">
							...
							<label htmlFor="filterToggle" id="filterIcon">
								<svg viewBox="0 0 19 16" onClick={() => {
									this.setState({ toggledFilter: !this.state.toggledFilter });
								}}>
									<path
				...
				<form id='filter' style={{ display: this.state.toggledFilter ? 'flex' : 'none' }}>
					<div id='type'>
						<h6>Type</h6>
